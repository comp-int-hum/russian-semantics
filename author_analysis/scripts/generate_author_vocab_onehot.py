...
def sorted_auth_iterator(auth_mat):
    ...
    for key, value in sorted(auth_mat.items(), key=lambda x: (x[0][3], x[0][1], x[0][0], x[0][2])):
        yield key + (value,)

def sorted_topic_iterator(topic_mat):
    # Sort by (win_id, topic_id, vocab_id)
    for key, value in sorted(topic_mat.items(), key=lambda x: (x[0][2], x[0][0], x[0][1])):  
        yield key + (value,)
...
if __name__ == '__main__':
    ...
    args.device = args.device if args.device else torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    word_list, num_topics, num_windows = model.word_list, model.num_topics, model.num_windows
    ...
    with gzip.open(args.input_dir, "rt") as ifd:
        for auth_id, line in tqdm(enumerate(ifd)):
            ...
            localword2localid, work2id = data[args.word2id_field], data[args.work2id_field]
            num_vocab, num_work, num_topic = embed_data.shape
            id2work = {v : k for k, v in work2id.items()}
            localid2id = {localword2localid[w] :word2id[w] for w, _ in localword2localid.items()}
            ...
            for work_id in range(num_work):
                ...
                for localvocab_id, vocab_id in localid2id.items():
                    for topic_id in range(num_topic):
                        per_vocab_top_counter = embed_data[localvocab_id, work_id, topic_id]
                        if per_vocab_top_counter > 0:
                            auth_mat[(auth_id, topic_id, vocab_id, win_id)] += per_vocab_top_counter 
                            topic_mat[(topic_id, vocab_id, win_id)] += per_vocab_top_counter
