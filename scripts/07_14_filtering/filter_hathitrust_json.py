...
DATA_DIR = (
    "/home/zxia15/data_zxia15/russian-semantics/work/full_russian_documents.jsonl.gz"
)
...
def check_cyrillic_dicts() -> None:
    for _, value in CYRILLIC_UNICODE.items():
        ...
        print(
            f"the range encompasses {ord(upper_b)- ord(lower_b) + 1} code points but is only assigned {range_num} code points"
        )
...
def re_filtering(data: str) -> str:
    ...
    pattern = re.compile(
        r"[\u0049\u0056\u0058\u004C\u0043\u0044\u004D\u0000-\u0040\u005B-\u0060\u007B-\u007F\u0400-\u04FF\u0500-\u052F\u2DE0-\u2DFF\uA640-\uA69F\u1C80-\u1C8F\u1D2B-\u1D78\uFE2E-\uFE2F]*"
    )
...
if __name__ == "__main__":
    ...
    data_generator = (
        open_jsonl_file(DUMP_RE_DATA_DIR)
        if DEBUG_MODE is True
        else open_jsonl_file(DATA_DIR)
    )
    idx: int = 0
    # 从这里开始，别改了！！
    prior_idx: int = 89353
    ...
    try:
        if DEBUG_MODE is True:
            while True:
                ...
                if idx % 50 == 0:
                    print(
                        f"record #{idx}: has russian percentage {record['russian text ratio']}%"
                    )
                idx += 1
