import argparse, json, re, unicodedata, Levenshtein
import pandas as pd
...
def merge_two_instances(str_01: str, str_02 : str, similarity_flag : bool, 
    ...
    if similarity_flag is True:
        group_01: int = author_correspondence_dict.get(str_01, None)
        group_02: int = author_correspondence_dict.get(str_02, None)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-a", "--author_column", type=str, default="author")
    parser.add_argument("-t", "--title_column", type=str, default="title")
    parser.add_argument("-d", "--debug", action="store_true")
    parser.add_argument("-la", "--author_levratio", type=float, default=1.0)
    parser.add_argument("-lt", "--title_levratio", type=float, default=1.0)
    parser.add_argument("-f", "--title_filter_criteria", type=str, default="year")
    parser.add_argument("-fa", "--filter_ascending", action="store_false")
    ...
    print(f"opening input file at {args.input_dir}")
    if args.input_dir.endswith('.csv'):
        df = pd.read_csv(args.input_dir)
    elif args.input_dir.endswith('.jsonl.gz'):
        df = pd.read_json(args.input_dir, lines=True, compression='gzip')
    else:
    ...
    df = df[~df[args.author_column].isna()].reset_index().drop(['index'], axis=1)
    df = df[~df[args.title_column].isna()].reset_index().drop(['index'], axis=1)
    ...
    df["regularized_author"] = df[args.author_column].map(lambda x: preprocess_str(x, is_title=False))
    df["regularized_title"] = df[args.title_column].map(lambda x: preprocess_str(x, is_title=True))
    ...
    begin_next_step(explanation="filtering out author names using Levenshtein similarity",
                    turn_json=args.debug, json_data = author_name_grouping_by_abbre)
    ...
    for abbrev, author_arr in tqdm(author_name_grouping_by_abbre.items()):
        ...
        for i in range(len(author_arr) - 1):
            for j in range(i + 1, len(author_arr)):
                ...
                if last_name_sim_ratio > args.author_levratio:
                    ...
                    for idx in range(shorter_length):
                        ...
                        # non abbreviation
                            ...
                            if (Levenshtein.ratio(
                                other_name_01, 
                                other_name_02) < args.author_levratio):
    ...
    begin_next_step(explanation="organize the previously grouped authors",
                    turn_json=args.debug, json_data = author_name_alignment_01)
    ...
    begin_next_step(explanation="normalize all author names",
                    turn_json=args.debug, json_data = author_name_alignment_02)
    ...
    for author_name, title_arr in tqdm(title_name_grouping_by_author.items()):
        ...
        for i in range(len(title_arr) - 1):
            for j in range(i + 1, len(title_arr)):
                ...
                similarity_flag = (sim_ratio > args.title_levratio)
    ...
    print(f"filtering based on column {args.title_filter_criteria} in " + 
          f"{'ascending' if args.filter_ascending is True else 'descending'} order")
    ...
    for unique_instance in tqdm(unique_title_plus_author_instances):
        filtered_indices.append(
            df[df["complete_work_info"] == unique_instance]
                .sort_values(by=args.title_filter_criteria, ascending=args.filter_ascending)
                .index[0]
            )
    ...
    print(f"complete filtering. writing to {args.output_dir}")
